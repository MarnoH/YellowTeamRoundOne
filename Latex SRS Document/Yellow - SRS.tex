...
Users will be allowed to contribute to the crowdsourcing nature of NavUP by being able to report events such as protest action, and long queues at entrance gates or restaurants.
	...
	\begin{itemize}
		\item Using NavUP, users will be able to search for a desired location. Depending on search criteria , the application will display the desired location on a map of campus, or generate an optimal route between the user's current and desired location. 
		\item Students and Staff will be able to create custom timetables that will generate optimal routes between locations automatically.
		\item Based on user interests, users can be notified of events (created by students and staff) happening on campus.
		\item Users will be able to view leaderboards and earn achievements through the use of NavUP
	\end{itemize}
